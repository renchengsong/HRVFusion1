...
class DiffuSELearner:
  ...
  def train_step(self, features):
    ...
    with self.autocast:
      t = torch.randint(0, len(self.params.noise_schedule), [N], device=gt.device)
      noise_scale = self.noise_level[t].unsqueeze(1)
      noise_scale_sqrt = noise_scale**0.5
      m = (((1-self.noise_level[t])/self.noise_level[t]**0.5)**0.5).unsqueeze(1)
      noise = torch.randn_like(gt)
      noisy_bvp = (1-m) * noise_scale_sqrt * gt + m * noise_scale_sqrt * chrom + (1.0 - (1 + m ** 2) * noise_scale) ** 0.5 * noise #Xt
      combine_noise = (m * noise_scale_sqrt * (chrom - gt) + (1.0 - (1 + m ** 2) * noise_scale) ** 0.5 * noise) / (1 - noise_scale) ** 0.5

      predicted = self.model(noisy_bvp, spectrogram, t)
  ...
  def _write_summary(self, step, features, loss):
    ...
    writer.add_audio('feature/gt', features['gt'][0], step, sample_rate=self.params.sample_rate)
...
def train(args, params):
  dataset = dataset_from_path(args.gt_dir, args.chrom_dir,args.data_dirs, params)
...
def train_distributed(replica_id, replica_count, port, args, params):
  ...
  dataset = dataset_from_path(args.gt_dir, args.chrom_dir,args.data_dirs,params,is_distributed=True)
